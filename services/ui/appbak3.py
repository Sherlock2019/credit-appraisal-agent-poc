...
with tab_clean:
    ...
    st.markdown("Upload your **real CSV**. We’ll auto-detect and mask PII before you run the agent.")
    ...
    if st.button("🚀 Run Agent", use_container_width=True):
        try:
            ...
            def prep_and_pack(df: pd.DataFrame, filename: str):
                ...
                safe = to_agent_schema(safe)     # <-- ensure required columns for agent
                buf = io.StringIO()
            ...
            if data_choice == "Use sample dataset":
                ...
                files = prep_and_pack(safe_df, "synthetic_raw_masked.csv")
