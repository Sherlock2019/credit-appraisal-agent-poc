...
IMAGE_UPLOAD_TYPES = tuple(sorted({ext.lstrip(".") for ext in IMAGE_EXTS}))
...
hero_path = _first_existing_image(LANDING_HERO_BASENAME)
if not display_full_width_banner(hero_path):
...
if hero_upload is not None:
    ...
    if saved_path:
        st.success("✅ Landing banner updated")
...
for sector, industry, agent, desc, status, emoji in AGENTS:
    ...
    image_html = render_image_tag(agent_key, industry, emoji)
    rows.append(
        {
            "🖼️": image_html,
            "🏭 Sector": sector,
...
def render_tab_run():
    ...
    if available_agents:
        ...
        default_agent = st.session_state.get("selected_agent_label")
        default_index = 0
        if default_agent in agent_labels:
            default_index = agent_labels.index(default_agent)
        ...
        st.session_state["selected_agent_label"] = selected_agent_label
...
def render_tab_review():
    ...
    if "last_merged_df" not in st.session_state:
        ...
        st.download_button("⬇️ Export review CSV", csv_bytes, review_name, "text/csv", use_container_width=True)
...
def render_tab_train():
    ...
    with colB:
        if st.button("⬆️ Promote last candidate to PRODUCTION", use_container_width=True):
            try:
